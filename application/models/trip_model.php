...
class Trip_model extends CI_Model {
	...
	function get_sql_for_trips($condition=false){
		...
		$qry='SELECT TV.payment_type_id as payment_type_id,ORG.name as company_name ,VO.name as ownership,T.id,T.customer_id,T.customer_group_id,T.remarks,T.vehicle_model_id,T.vehicle_ac_type_id,T.driver_id,T.vehicle_id,T.guest_id,V.vehicle_ownership_types_id,T.tariff_id,T.trip_status_id,T.id as trip_id,T.booking_date,T.drop_date,T.drop_time,T.pick_up_date,T.pick_up_time,VM.name as model,Substr(V.registration_number,-4) as registration_number,T.pick_up_city,T.pick_up_area,IFNULL(T.guest_name,G.name) as guest_name,IFNULL(T.guest_mobile,G.mobile) as guest_info,T.drop_city,T.drop_area,C.name as customer_name,C.mobile as customer_mobile,CG.name as customer_group,D.name as driver,D.mobile as driver_info, D.driver_status_id,TV.invoice_no,TV.delivery_no,TV.voucher_no
