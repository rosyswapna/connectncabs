...
class Trip_booking_model extends CI_Model {
	...
	function getTripVoucher($trip_id){
		...
		$this->db->select('ORG.name as company_name ,VO.name as ownership,T.customer_id,T.vehicle_id,T.advance_amount,T.customer_group_id,T.remarks,T.vehicle_model_id, T.vehicle_ac_type_id,T.vehicle_ac_type_id,T.driver_id,T.vehicle_id,T.guest_id,V.vehicle_ownership_types_id,T.tariff_id,T.trip_status_id,T.id as trip_id,T.booking_date,T.drop_date,T.drop_time,T.pick_up_date,T.pick_up_time,T.advance_amount,VM.name as model,V.registration_number,T.pick_up_city,T.pick_up_area,IFNULL(G.name,T.guest_name) as guest_name,G.mobile as guest_info,T.drop_city,T.drop_area,C.name as customer_name,C.mobile as customer_mobile,CG.name as customer_group,D.name as driver,D.mobile as driver_info,D.driver_status_id,OW.name as supplier');
		$this->db->from('trips T');
		$this->db->join('vehicle_models VM','VM.id=T.vehicle_model_id','left');
		$this->db->join('vehicle_owners OW','OW.id=T.supplier_group_id','left');
...
AND CURDATE() BETWEEN T.pick_up_date AND T.drop_date';
 ...
 if(count($results)>0){
	...
	$org_id = $this->session->userdata('organisation_id');
		...
		$qry='SELECT IFNULL(T.guest_name,G.name) as guest_name,IFNULL(T.guest_mobile,G.mobile) as guest_info,C.name as customer_name,C.mobile as customer_info,T.guest_id,T.customer_id,T.drop_city,T.pick_up_date,T.pick_up_time,T.pick_up_city,D.name as driver,D.mobile as driver_info,ORG.name AS org_name,VM.name AS v_model 
			FROM trips T  
			...
			where T.organisation_id='.$org_id.' AND T.id='.$id; 
